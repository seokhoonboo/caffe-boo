...
namespace caffe {
	...
	void StochasticReNormLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
		StochasticReNormParameter param = this->layer_param_.stochastic_renorm_param();
		...
		mean_rng_ = param.mean_rng();
		std_rng_ = param.std_rng();
	...
	template <typename Dtype>
	void StochasticReNormLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
		if (bottom[0]->num_axes() >= 1)
	...
	template <typename Dtype>
	void StochasticReNormLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
		...
		if (!use_global_stats_) {
			...
			if (step >= step_to_init_ && first_iter_in_step)
			{
				...
				caffe_rng_uniform(variance_.count(), Dtype(-1), Dtype(1), mean_.mutable_cpu_diff());
				caffe_rng_uniform(variance_.count(), Dtype(-1), Dtype(1), variance_.mutable_cpu_diff());
				caffe_mul(variance_.count(), this->blobs_[1]->cpu_diff(), mean_.cpu_diff(), mean_.mutable_cpu_diff());
				caffe_mul(variance_.count(), this->blobs_[1]->cpu_diff(), variance_.cpu_diff(), variance_.mutable_cpu_diff());
				caffe_scal(variance_.count(), mean_rng_, mean_.mutable_cpu_diff());
				caffe_scal(variance_.count(), std_rng_, variance_.mutable_cpu_diff());
				caffe_add(variance_.count(), this->blobs_[0]->cpu_diff(), mean_.mutable_cpu_diff(), this->blobs_[0]->mutable_cpu_diff());
				caffe_add(variance_.count(), this->blobs_[1]->cpu_diff(), variance_.mutable_cpu_diff(), this->blobs_[1]->mutable_cpu_diff());
			}
	...
	template <typename Dtype>
	void StochasticReNormLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
	STUB_GPU(StochasticReNormLayer);
#endif

	INSTANTIATE_CLASS(StochasticReNormLayer);
	REGISTER_LAYER_CLASS(StochasticReNorm);
}  // namespace caffe
